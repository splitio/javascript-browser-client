...
export default function (fetchMock, assert) {
    ...
    fetchMock.postOnce(url(settings, '/testImpressions/bulk'), (url, opts) => {
      setTimeout(() => {
        window.ga(() => {
          ...
          setTimeout(() => {
            t.ok(logSpy.calledWith(`[WARN]  splitio => SplitToGa queue method threw: ${error}. No hit was sent.`));
            t.ok(logSpy.calledWith('[WARN]  splitio => your custom mapper returned an invalid FieldsObject instance. It must be an object with at least a `hitType` field.'));
            client.destroy();
            logSpy.restore();
            t.end();
          });
  ...
  assert.test(t => {
    ...
    t.ok(logSpy.calledWith('[WARN]  splitio => `ga` command queue not found. No hits will be sent until it is available.'), 'warning GA not found');
