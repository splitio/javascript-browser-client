...
tape('Split Suite: Browser SDK & RUM Agent', async function (assert) {
  const config = {
    core: {
      ...
      key: 'ut1',
      trafficType: 'tt1' // for RUM-Agent identity
    },
  ...
  assert.deepEqual(window.SplitRumAgent.getIdentities(), [{ key: config.core.key, trafficType: 'tt1' }], 'Identity for the main client.');

  const client2 = suite.client('ut2');
  const client3 = suite.client('ut3', 'tt2');
  assert.deepEqual(window.SplitRumAgent.getIdentities(), [{
    key: config.core.key, trafficType: 'tt1'
  }, {
  ...
  assert.deepEqual(parsedPayload.entries.map(event => ({ eventTypeId: event.eventTypeId, key: event.key, trafficTypeName: event.trafficTypeName })), [
    { trafficTypeName: 'tt1', key: 'ut1', eventTypeId: 'prefix.custom-event-for-all-clients' },
    ...
    { trafficTypeName: 'tt1', key: 'ut1', eventTypeId: 'prefix.time.to.dom.interactive' },
    ...
    { trafficTypeName: 'tt1', key: 'ut1', eventTypeId: 'prefix.page.load.time' },
